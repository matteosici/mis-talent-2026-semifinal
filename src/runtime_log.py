...
def build_sample_runtime_log(decision_card: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        event(agent_name="Finance & Data Agent", event_type="observe", tool_or_api_id="TEAM_PACK_EXCEL", request_id="REQ-FIN-001", response_status="8_core_sheets_loaded"),
        ...
        event(agent_name="Decision & Partner Agent", event_type="reflection", tool_or_api_id="SCHEMA_VALIDATION", request_id="REQ-DEC-001", masked_fields=decision_card.get("masked_fields", []), response_status=llm_meta.get("schema_validation", "PASSED"), human_approval_id=decision_card.get("human_approval_id")),
    ]
